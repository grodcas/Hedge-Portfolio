...
export function parseItem2(html) {
  ...
  walk(root, (node) => {
    ...
    // Detect boundaries
      ...
      if (in2 && isJNJ10QItem1End(node, $, direct)) {
        stop = true;
        return;
      }
    if (stop) return;
...
/*
// CLI
if (process.argv[1].includes("JNJ_10Q_parse_item2.js")) {
  const html = fs.readFileSync("edgar_raw_html/JNJ_10-Q_2025-08-01.html","utf8");
  const out = parseItem2(html);
  fs.writeFileSync("edgar_parsed_json/JNJ_10-Q_2025-08-01_item2.json", JSON.stringify(out,null,2));
  console.log("Saved JNJ_10K_item2.json");
}
*/