...
function isNTFXItem1Start(node, $, text) {
  if (!text.includes("Consolidated Statements of Operations")) return false;
...
function isNTFXItem1End(node, $, text) {
...
export function parseItem1(html) {
  ...
  walk(root, (node) => {
    ...
    // Detect boundaries
    if (isNTFXItem1Start(node, $, direct)) {
      in1 = true;
      return;
    }

    if (in1 && isNTFXItem1End(node, $, direct)) {
      stop = true;
      return;
    }
    if (stop) return;
...
/*
// CLI
if (process.argv[1].includes("NFLX_10Q_parse_item1.js")) {
  const html = fs.readFileSync("edgar_raw_html/NFLX_10-Q_2025-08-01.html","utf8");
  const out = parseItem1(html);
  fs.writeFileSync("edgar_parsed_json/NFLX_10-Q_2025-08-01_item1.json", JSON.stringify(out,null,2));
  console.log("Saved NFLX_10K_item1.json");
}
*/