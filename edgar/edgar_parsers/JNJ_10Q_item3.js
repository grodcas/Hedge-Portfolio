...
function isJNJ10QItem1Start(node, $, text) {
  if (!text.includes("Item 3")) return false;
...
function isJNJ10QItem1End(node, $, text) {
  if (!text.includes("Item 4")) return false;
  if (node.tagName.toLowerCase() !== "span") return false;
...
export function parseItem3(html) {
  ...
  walk(root, (node) => {
    ...
    if (in3 && isJNJ10QItem1End(node, $, direct)) {
      stop = true;
      return;
    }
    if (stop) return;
...
/*
// CLI
if (process.argv[1].includes("JNJ_10Q_parse_item3.js")) {
  const html = fs.readFileSync("edgar_raw_html/JNJ_10-Q_2025-08-01.html","utf8");
  const out = parseItem3(html);
  fs.writeFileSync("edgar_parsed_json/JNJ_10-Q_2025-08-01_item3.json", JSON.stringify(out,null,2));
  console.log("Saved JNJ_10K_item3.json");
}
*/