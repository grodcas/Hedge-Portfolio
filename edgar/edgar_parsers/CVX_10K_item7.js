...
function isCVXItem7Start(text) {
  return text.includes("Key Financial Results");
}

// This is the FIRST financial statement table header
function isCVXItem7End(text) {
  const t = clean(text);
  return text.includes("To the Stockholders of Chevron Corporation") ;
}
  ...
  export function parseItem7(html) {
    ...
    const fullText = clean($(node).text());
      ...
      if (isCVXItem7Start(text)) {
        in7 = true;
        return;
      }

      if (in7 && isCVXItem7End(text)) {
        stop = true;
        return;
      }
...
/*
// CLI
if (process.argv[1].includes("CVX_10K_parse_item7.js")) {
  const html = fs.readFileSync("edgar_raw_html/CVX_10-K_2023-11-03.html","utf8");
  const out = parseItem7(html);
  fs.writeFileSync("edgar_parsed_json/CVX_10-K_2023-11-03_item7.json", JSON.stringify(out,null,2));
  console.log("CVX_10-K_2023-11-03_item7.json");
}
*/