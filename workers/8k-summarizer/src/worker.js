...
`.trim();
    ...
    if (clusters.length === 1) {
      const s = await summarize(eightKPrompt(clusters[0].content));
    ...
    for (const c of clusters) {
      const s = await summarize(eightKPrompt(c.content));
...
Focus only on practical implications.
