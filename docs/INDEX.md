...
| [active/SPRINT_ai_agent_wiring.md](active/SPRINT_ai_agent_wiring.md) | Sprint plan — runs **after** pipeline implementation. Stands up all 25 AI agents from TICKER_PIPELINE + MAP_PIPELINE: discover existing worker infra, implement one worker per agent (model assignment table inside), build a smart orchestrator with per-agent trigger contracts, persist outputs as JSON columns, wire dashboard reads, run an evaluator agent + iterate prompts to analyst-grade before ship. Soft delete only. ~10–12h, splittable across two days. | Planned · sequential to pipeline impl |
| [active/SPRINT_validation_cleanup.md](active/SPRINT_validation_cleanup.md) | Sprint plan — runs **last**, after all three feature sprints have shipped. End-to-end smoke test, dead-code/orphan walk, DB schema reconciliation, doc lifecycle prune (move shipped sprints to archive, promote stable design docs to features/), bug log for follow-up. No new features, no refactors, soft delete only. ~3–5h. | Planned · sequential, final |
